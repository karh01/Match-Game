...
function createCards(parentElement, shuffledImageClasses) {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    let newCardElement = appendNewCard(parentElement);
     ...
     //newCardElement.querySelector(".card-down").classList.add(shuffledImageClasses[i]);
...
function doCardsMatch(cardObject1, cardObject2) {
  ...
  // 
  const imageClass1 = cardObject1.imageClass;
  const imageClass2 = cardObject2.imageClass;
  const match = imageClass1 === imageClass2;
...
function incrementCounter(counterName, parentElement) {
  ...
  /* Step 3: Change the HTML within 'parentElement' to display the new counter value. (possible if statement instead of just parentElement.textContent = counters[counterName];) */
    if (parentElement) {
        parentElement.textContent = counters[counterName];
    }
}
console.log(incrementCounterTest());
...
function onCardFlipped(newlyFlippedCard) {
  ...
  incrementCounter("flipCount", document.getElementById("flip-count"));
    ...
    let imageClass1 = lastCardFlipped.imageClass;
    let imageClass2 = newlyFlippedCard.imageClass;
